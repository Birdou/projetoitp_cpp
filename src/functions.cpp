...
std::string paintit::functions::plot(paintit::ppm &image, const paintit::penc &pincel, const std::string &expression)
{
	const ssize_t meta = image.getWidth() / 100;
	const ssize_t halfH = (image.getHeight() - 1) / 2;
	const ssize_t halfW = (image.getWidth() - 1) / 2;

	const paintit::penc axis(paintit::rgb(122, 122, 122), 1);

	line(image, axis, paintit::coord(halfW, 0), paintit::coord(halfW, image.getHeight()));
	line(image, axis, paintit::coord(0, halfH), paintit::coord(image.getWidth(), halfH));

	for (double i = -halfW, ri = 0; i < meta; i += 0.01, ++ri)
	{
		std::string ex_expression(expression);
		lib::replaceAll(ex_expression, "x", std::to_string(i));
		std::stringstream stream(lib::smath<double>(ex_expression));

		double nResult;
		stream >> nResult;

		ssize_t x = halfW + (i * 100);
		ssize_t y = halfH + (nResult * -100);

		if (y < image.getHeight() && y >= 0 && x < image.getWidth() && x >= 0)
			pincel.touch(image[x][y]);
	}
	return "";
}
